...
public class Multistream : Stream
{
    ...
    protected Multistream(Stream stream, params string[] protocols)
    {
        _stream = stream;
        _handshaker = new MultistreamHandshaker(this, /*Debugger.IsAttached ? TimeSpan.Zero :*/ TimeSpan.FromSeconds(3), protocols);
    }

    public static Multistream Create(Stream stream, string protocol) => new(stream, protocol);

    public static Multistream CreateSelect(Stream stream, string protocol)
